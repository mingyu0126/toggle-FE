...
- **`PublicInstitution` 엔티티**: 카카오 장소 ID(`externalPlaceId`)와 연동되는 공공기관 데이터를 관리합니다. `address`, `latitude`, `longitude`, `congestionLevel`, `waitTime`, `operatingHours`를 포함합니다.
...
- **공공기관 조회 (`POST /api/v1/public-institutions/lookup`)**:
    - 카카오 ID 목록 및 메타데이터를 받아 DB 내 엔티티로 해석하고 라이브 데이터를 반환합니다.
    - DB에 없는 경우 요청에서 온 메타데이터(주소, 좌표 등)를 기반으로 엔티티를 자동 생성하여 연동합니다.
- **즐겨찾기 관리**:
...
- **`useKakaoPlacesWithLookup` 훅**:
    ...
    - 검색된 장소의 메타데이터를 백엔드 lookup 시 함께 전송하여 DB의 최신성을 유지합니다.

### 3.2. 상태 관리 및 매핑 정책
- **Favorites**: 서버 데이터(`auth/me`)를 마스터로 사용하며, 변경 시 `favoritesChanged` 커스텀 이벤트를 통해 앱 전역의 UI(지도 마커, 리스트 배지 등)가 즉시 갱신됩니다.
- **Unified Mappers (`lib/mappers.js`)**: 
    - 백엔드 DTO 규격을 프론트엔드 표준 장소 객체로 변환하는 로직을 단일화했습니다.
    - `mapStoreToPlace`, `mapPublicToPlace`, `createMergedPreviewPlace`를 통해 데이터 일관성을 보장합니다.
...
- 점주 프로필 저장 시 불필요한 기본 이미지 데이터를 필터링하여 페이로드 크기와 DB 오염을 방지합니다.
